...
fetch('https://da-demo.github.io/api/futurama/episodes/')
    ...
    .then(episodes => {
        episodes.forEach(episode => {
            ...
            arrowDiv.classList.add('d-flex', 'flex-column', 'align-items-center');
            ...
            episodeDiv.appendChild(arrowDiv);
...
function showEpisodeDetails(episode, detailsElement) {
    ...
    detailsElement.innerHTML = `
        <p class="fs-6">Säsong: ${season}</p>
        <p class="fs-6">Avsnitt: ${episodeNumberInSeason}</p>
        <p class="fs-6">Sändningsdatum: ${episode.originalAirDate}</p>
    `;
}
...
document.getElementById('episodeForm').addEventListener('submit', function (event) {
    ...
    episodes.forEach(episode => {
        ...
        detailsElement.classList.add('episode-details', 'visually-hidden', 'col-md-9');
        detailsElement.innerHTML = `
            <p class="fs-6">Säsong: ${episode.season}</p>
            <p class="fs-6">Avsnitt: ${episode.episode}</p>
            <p class="fs-6">Sädningsdatum: ${episode.releaseDate}</p>
        `;
        episodeDiv.appendChild(detailsElement);
        episodeList.appendChild(episodeDiv);
    });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    episodes.forEach(episode => {
        ...
        detailsElement.innerHTML = `
            <p class="fs-6">Säsong: ${episode.season}</p>
            <p class="fs-6">Avsnitt: ${episode.episode}</p>
            <p class="fs-6">Sändningsdatum: ${episode.releaseDate}</p>
        `;
        episodeDiv.appendChild(detailsElement);
        episodeList.appendChild(episodeDiv);
    });
